    ...
    ? reviews.data.map((review: any) => (
        <div className="flex flex-col gap-4" key={review.id}>
          ...
          <div className="flex items-center gap-4 font-medium">
            <Image src={review.customer.avatar_url} alt="이미지" width={32} height={32} className="rounded-full" />
            <span>{review.customer.display_name}</span>
          </div>
          {/* STARS */}
          <div className="flex gap-2">
            {Array.from({ length: review.rating }).map((_, index) => (
              <Image src="/star.png" alt="별점" key={index} width={16} height={16} />
            ))}
          </div>
          {/* DESC */}
          ...
          <div className="">
            {review.media.map((media: any) => (
              <Image src={media.url} key={media.id} alt="이미지" width={100} height={50} className="object-cover" />
            ))}
          </div>
        </div>
      ))
